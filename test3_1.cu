...
__global__ void knn (double *oMatrix, double *topN, double*knns, unsigned matrixsize) {
  ...
  unsigned id = blockIdx.x * blockDim.x + threadIdx.x;
	...
	{
	// printf("\ncomparing %2f to %2f", oMatrix[i], topN[id]);
	 if(oMatrix[i] == topN[id]){
	 knns[id] = oMatrix[i+1];
	// printf("\nMATCH %2f to %2f", oMatrix[i], topN[id]);
...
__global__ void eucl_dist ( double *matrix, double *test, double *result, double *result2, unsigned matrixsize) {
  ...
  unsigned id = blockIdx.x * blockDim.x + threadIdx.x;
// printf("ID: %2d",id);
//printf("ID*Matrixsize: %2d",id*matrixsize);
...
//double iris2[4][5] = {{1, 2, 3, 4, 100},{5, 2, 5, 7, 100}, {6, 6, 1, 1, 200}, {7, 2, 3, 1, 300}};
//double test_iris[5] = {1, 2, 3.5, 4, 200};
//double test_iris[5] = {5.6,3.0,4.5,1.5,200};
//double test_iris[5] = {5.1,3.5,1.4,0.2, 100};
double test_iris[5] = {6.7,3.0,5.2,2.3,300};
 ...
 // square<<<1, block>>>(matrix, arr, result, M);
  ...
  cudaMemcpy(test,test_iris, (M)*sizeof(double), cudaMemcpyHostToDevice);
	  ...
	  //  for (unsigned i = 0; i < N; ++i) {
		// printf("%2f ", eucl_distance2[i]);
		// printf(" ");
	  //   }
	  // printf("Squared Matrix other: \n\n");
	  //  for (unsigned i = 0; i < 2*N; ++i) {
		// printf("%2f ", eucl_distance[i]);
		// printf(" ");
	  //   }
